...
/** Utility trait for puzzle solver.
  ...
  *   - data contained in an input file which can be parsed into type I by a
  *     parser function specific to each challenge
  ...
  *   - the parser function which takes an open file as a `BufferedSource` and
  *     output a challenge input of type I which can be used as is by the
  *     solution functions
  *   - two solution functions which solve the part 1 and part 2 of the
  *     challenge
  */
trait PuzzleSolver[I, O] extends IOApp.Simple:
  ...
  def parsedInput(inputFile: String): IO[I] =
    input(inputFile).use(source => IO(parse(source)))

  def solve1(inputFile: String): IO[O] = solve1(parsedInput(inputFile))

  def solve1(input: IO[I]): IO[O] = input.map(solutionPart1)
  ...
  override def run = printSolutions(inputFileName)
